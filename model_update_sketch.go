/*
Arduino IoT Cloud API

Provides a set of endpoints to manage Arduino IoT Cloud **Devices**, **Things**, **Properties** and **Timeseries**. This API can be called just with any HTTP Client, or using one of these clients:  * [Javascript NPM package](https://www.npmjs.com/package/@arduino/arduino-iot-client)  * [Python PYPI Package](https://pypi.org/project/arduino-iot-client/)  * [Golang Module](https://github.com/arduino/iot-client-go)

API version: 2.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v3

import (
	"encoding/json"
)

// checks if the UpdateSketch type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UpdateSketch{}

// UpdateSketch struct for UpdateSketch
type UpdateSketch struct {
	// The autogenerated sketch version
	SketchVersion *string `json:"sketch_version,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _UpdateSketch UpdateSketch

// NewUpdateSketch instantiates a new UpdateSketch object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUpdateSketch() *UpdateSketch {
	this := UpdateSketch{}
	return &this
}

// NewUpdateSketchWithDefaults instantiates a new UpdateSketch object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUpdateSketchWithDefaults() *UpdateSketch {
	this := UpdateSketch{}
	return &this
}

// GetSketchVersion returns the SketchVersion field value if set, zero value otherwise.
func (o *UpdateSketch) GetSketchVersion() string {
	if o == nil || IsNil(o.SketchVersion) {
		var ret string
		return ret
	}
	return *o.SketchVersion
}

// GetSketchVersionOk returns a tuple with the SketchVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UpdateSketch) GetSketchVersionOk() (*string, bool) {
	if o == nil || IsNil(o.SketchVersion) {
		return nil, false
	}
	return o.SketchVersion, true
}

// HasSketchVersion returns a boolean if a field has been set.
func (o *UpdateSketch) HasSketchVersion() bool {
	if o != nil && !IsNil(o.SketchVersion) {
		return true
	}

	return false
}

// SetSketchVersion gets a reference to the given string and assigns it to the SketchVersion field.
func (o *UpdateSketch) SetSketchVersion(v string) {
	o.SketchVersion = &v
}

func (o UpdateSketch) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UpdateSketch) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.SketchVersion) {
		toSerialize["sketch_version"] = o.SketchVersion
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *UpdateSketch) UnmarshalJSON(data []byte) (err error) {
	varUpdateSketch := _UpdateSketch{}

	err = json.Unmarshal(data, &varUpdateSketch)

	if err != nil {
		return err
	}

	*o = UpdateSketch(varUpdateSketch)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "sketch_version")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableUpdateSketch struct {
	value *UpdateSketch
	isSet bool
}

func (v NullableUpdateSketch) Get() *UpdateSketch {
	return v.value
}

func (v *NullableUpdateSketch) Set(val *UpdateSketch) {
	v.value = val
	v.isSet = true
}

func (v NullableUpdateSketch) IsSet() bool {
	return v.isSet
}

func (v *NullableUpdateSketch) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUpdateSketch(val *UpdateSketch) *NullableUpdateSketch {
	return &NullableUpdateSketch{value: val, isSet: true}
}

func (v NullableUpdateSketch) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUpdateSketch) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


