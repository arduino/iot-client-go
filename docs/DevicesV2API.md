...
> []ArduinoDevicev2 DevicesV2List(ctx).AcrossUserIds(acrossUserIds).Serial(serial).ShowDeleted(showDeleted).Tags(tags).UniqueHardwareId(uniqueHardwareId).XOrganization(xOrganization).Execute()
